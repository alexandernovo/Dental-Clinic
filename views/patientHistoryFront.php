...
<?php $user = first('patient', ['Patient_ID' => $_SESSION['Patient_ID']]); ?>
<?php $appoint_result = firstJoin('appointmentresult', [['appointment', 'appointment.Appointment_ID', 'appointmentresult.Appointment_ID']], ['appointment.Patient_ID' => $_SESSION['Patient_ID']], ['appointment.Appointment_ID' => "DESC"]); ?>

<?php if (!empty($appoint_result)) { ?>
    <?php $appoint_prescript = firstJoin('appointmentresult', [['appointment', 'appointment.Appointment_ID', 'appointmentresult.Appointment_ID'], ['prescription', 'prescription.Appointment_ID', 'appointment.Appointment_ID']], ['appointment.Patient_ID' => $_SESSION['Patient_ID']], ['appointment.Appointment_ID' => "DESC"]); ?>
    <div class="col-lg-12 grid-margin stretch-card mb-4">
        <div class="card px-2 pb-2">
            <div class="card-body">
                <div class="row mx-auto mt-3">
                    <div class="">
                        <a href="patientViewHistory.php?from=<?php echo date('Y-m-d'); ?>&to=<?php echo date('Y-m-d', strtotime('+7 days')); ?>" class="btn float-end btn-primary mb-3"><i class="fa fa-history"></i> History</a>
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="card shadow border">
                        ...
                        <div class="card-body p-4 d-flex justify-content-center position-relative mx-auto" id="teeth_box" patient-id="<?php echo $_SESSION['Patient_ID']; ?>" style="height:610px">
                            <?php
                            for ($i = 1; $i <= 32; $i++) {
                                ${"teeth{$i}"} = firstJoin('tooth', [['appointmenttoothresult', 'tooth.Tooth_ID', 'appointmenttoothresult.Tooth_ID'], ['appointment', 'appointment.Appointment_ID', 'appointmenttoothresult.Appointment_ID']], ['tooth.Patient_ID' => $_SESSION['Patient_ID'], 'tooth.Tooth_Number' => $i], ['appointment.Patient_ID' => 'DESC']);
                            }
                            ?>
                            <?php
                            for ($i = 1; $i <= 32; $i++) {
...
<?php } else { ?>
    <h1 class="text-center text-secondary mt-3">No History Yet</h1>
<?php } ?>
