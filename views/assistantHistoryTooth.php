...
<?php $appoint_result = firstJoin('appointmentresult', [['appointment', 'appointment.Appointment_ID', 'appointmentresult.Appointment_ID']], ['appointment.Appointment_ID' => $_GET['appointment_id']]); ?>
<?php $appoint_prescript = firstJoin('appointmentresult', [['appointment', 'appointment.Appointment_ID', 'appointmentresult.Appointment_ID'], ['prescription', 'prescription.Appointment_ID', 'appointment.Appointment_ID']], ['appointment.Appointment_ID' => $_GET['appointment_id']]); ?>
        ...
        <div class="card-body">
            <div class="row mx-auto mt-3">
                <div class="d-flex justify-content-between align-items-center">
                    <h4 class="card-title m-0"><?= $user['patientFname'] . ' ' . $user['patientLname'] ?> Appointment History</h4>
                    <div class="col-md-2">
            ...
            <div class="col-md-6">
                <div class="card shadow border">
                    ...
                    <div class="card-body p-4 d-flex justify-content-center position-relative mx-auto" id="teeth_box" patient-id="<?php echo $_GET['patient_id']; ?>" style="height:610px">
                        <?php
                        for ($i = 1; $i <= 52; $i++) {
                            ${"teeth{$i}"} = firstJoin('tooth', [['appointmenttoothresult', 'tooth.Tooth_ID', 'appointmenttoothresult.Tooth_ID']], ['tooth.Patient_ID' => $_GET['patient_id'], 'Appointment_ID' => $_GET['appointment_id'], 'tooth.Tooth_Number' => $i]);
                        }
                        ?>

                        <div class="popup" id="popup">
                            <div class="row mx-auto mb-1">
                                <button class="btn btn-primary btn-sm tooths-class tootie" id="normal-tooth" tooth-type="0" style="display: none;" appointment-id="<?php echo $_GET['appointment_id'] ?>" patient-id="<?php echo $_GET['patient_id'] ?>">Normal</button>
                            </div>
                            <div class="row mx-auto mb-1">
                                <button class="btn btn-primary btn-sm tooths-class tootie" tooth-type="1" appointment-id="<?php echo $_GET['appointment_id'] ?>" patient-id="<?php echo $_GET['patient_id'] ?>">Filled</button>
                            </div>
                            <div class="row mx-auto mb-1">
                                <button class="btn btn-primary btn-sm tooths-class tootie" tooth-type="2" appointment-id="<?php echo $_GET['appointment_id'] ?>" patient-id="<?php echo $_GET['patient_id'] ?>">Cavity</button>
                            </div>
                            <div class="row mx-auto mb-1">
                                <button class="btn btn-primary btn-sm tooths-class tootie " tooth-type="3" appointment-id="<?php echo $_GET['appointment_id'] ?>" patient-id="<?php echo $_GET['patient_id'] ?>">Extracted</button>
                            </div>
                            <div class="row mx-auto">
                                <button class="btn btn-primary btn-sm tooths-class tootie" tooth-type="4" appointment-id="<?php echo $_GET['appointment_id'] ?>" patient-id="<?php echo $_GET['patient_id'] ?>">Replaced</button>
                            </div>
                        </div>
                        ...
                        for ($i = 1; $i <= 52; $i++) {
                            $tooth = "teeth" . $i;
                            $tooth_data = ${'teeth' . $i};
                            echo '<img class="teeth ' . $tooth . ($tooth_data['Recent_change'] == 1 ? ' border-custom' : '') . '" tooth-status="' . $tooth_data['Appointment_Tooth_Status'] . '" tooth-id="' . $tooth_data['AppointmentToothResult_ID'] . '" src="';
